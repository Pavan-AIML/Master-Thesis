...
from notebooks.Modis_data_analysis.data_loader_analysis import Modis_data_loader
from notebooks.Modis_data_analysis.data_loader_analysis import Training_data_loader
...
print([name for name, _ in inspect.getmembers(Training_data_instance, predicate=inspect.ismethod)])
...
Nearest_neighbours_data = Training_data_instance.PM25_nearest_neighbour_finder(Complete_AOD_data)

Nearest_neighbours_data.head(), Nearest_neighbours_data.shape, type(Nearest_neighbours_data)

Cleaned_AOD_data = Training_data_instance.data_cleaning_and_missing_values_handeling(Nearest_neighbours_data)
Cleaned_AOD_data.head(), Cleaned_AOD_data.shape, type(Cleaned_AOD_data)

Final_training_data = Training_data_instance.get_final_training_data(Cleaned_AOD_data,daily_PM25_data)
