import torch
import torch.nn as nn
import torch.nn.functional as F

""" 
From here as we can see that the input to this module will be the output of the Encoder module which is r.
"""


class Latent_Encoder(nn.Module):
    def __init__(self, hidden_dim, latent_dim):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.latent_dim = latent_dim
        self.mu = nn.Linear(self.hidden_dim, self.latent_dim)
        self.log_var = nn.Linear(self.hidden_dim, self.latent_dim)

    def forward(self, r):
        mu = self.mu(r)
        log_var = self.log_var(r)
        # vectors of latent dimension.
        return mu, log_var
    

class Robust_Latent_Encoder(nn.Module):
    def __init__(self, hidden_dim, latent_dim):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.latent_dim = latent_dim
        self.net = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim ),
            nn.ReLU(),
            nn.LayerNorm(self.hidden_dim),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU()
        )
        self.mu = nn.Linear(self.hidden_dim, self.latent_dim)
        self.log_var = nn.Linear(self.hidden_dim, self.latent_dim)
        
    def forward(self, r):
        hidden = self.net(r)
        mu = self.mu(hidden)
        log_var = self.log_var(hidden)
        
        # Stability: clamp log_var to prevent numerical uncertainty issue during sampling. 
        log_var = torch.clamp(log_var, -5,5)
        # To make the var more stable we will do the clamping here
        return mu, log_var
        
        
    
    



