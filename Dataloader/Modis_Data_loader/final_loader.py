...
# AirQualityDataset_latlon_AOD_PM25_n = Airqualitydataset()


class Final_Air_Quality_Dataset_pipeline:
    def __init__(self, config, geospatial_encoder,input_type=1, output_type=1,  flag="Train"):
        self.config = config
        self.flag = flag
    ...
    def Torch_data(self):
        self.torch_data = Airqualitydataset(
            self.Training_data,
            self.config,
            flag=self.flag,
    ...
    def full_pipeline(self):
        modis_data_sets = self.modis_data_sets()
        stations_data = self.stations_data_sets()
        pm25_data = self.PM_25_data()
        training_data = self.training_data()
        torch_data = self.Torch_data()
        torch_latlong_loader = DataLoader(
            torch_data, batch_size=len(torch_data), shuffle=False
        )
        for inputs, outputs in torch_latlong_loader:
            all_latlong = inputs[:, 0:2]
        all_latitude_longitude_embeddings = self.geospatial_encoder(
            all_latlong
        )
